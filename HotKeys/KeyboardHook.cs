        ...
        private int HookProc(int nCode, IntPtr wParam, ref KeyboardLLHookStruct lParam)
        {
            if (nCode >= 0)
            {
                ...
                if (wParam.ToInt32() == WM_KEYDOWN || wParam.ToInt32() == WM_SYSKEYDOWN)
                {
                    ...
                    if ((int)Settings.SelectMonitor.Key3 == lParam.vkCode)
                    {
                        var key1 = true;
                        var key2 = true;

                        if (Settings.SelectMonitor.Key1 != VirtualKeyModifier.None)
                        {
                            var key1State = GetAsyncKeyState((int)Settings.SelectMonitor.Key1) & 0x8000;
                            key1 = Convert.ToBoolean(key1State);
                        }

                        if (Settings.SelectMonitor.Key2 != VirtualKeyModifier.None)
                        {
                            var key2State = GetAsyncKeyState((int)Settings.SelectMonitor.Key2) & 0x8000;
                            key2 = Convert.ToBoolean(key2State);
                        }
