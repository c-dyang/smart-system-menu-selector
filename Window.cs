    ...
    class Window : IDisposable
    {
        ...
        public void MoveToMonitor(IntPtr monitorHandle)
        {
            ...
            if (currentMonitorHandle != monitorHandle)
            {
                ...
                if (wasMaximized)
                {
                    // 最大化窗口：先还原 → 移动到目标屏 → 重新最大化（保持状态）
                    var placement = new WINDOWPLACEMENT();
                    placement.length = Marshal.SizeOf(placement);
                    placement.showCmd = 9; // SW_RESTORE
                    placement.rcNormalPosition.Left = left;
                    placement.rcNormalPosition.Top = top;
                    placement.rcNormalPosition.Right = left + windowRect.Width;
                    placement.rcNormalPosition.Bottom = top + windowRect.Height;
                    SetWindowPlacement(Handle, ref placement);

                    var maximizedPlacement = new WINDOWPLACEMENT();
                    maximizedPlacement.length = Marshal.SizeOf(maximizedPlacement);
                    maximizedPlacement.showCmd = 3; // SW_MAXIMIZE
                    maximizedPlacement.rcNormalPosition = placement.rcNormalPosition;
                    SetWindowPlacement(Handle, ref maximizedPlacement);
                }
                else
                {
